      ...
      fabric.FabricImage.fromURL(imgEl.src, { crossOrigin: "anonymous" })
        .then((img) => {
          ...
          if (imgEl.gradePreset && imgEl.gradePreset !== "none") {
            ...
            if (matrix) {
              filters.push(new fabric.filters.ColorMatrix({ matrix }));
            }
          }
          
          // Film Grain Engine
          if (imgEl.filmGrain && imgEl.filmGrain > 0) {
            // Noise values 0-100 mapped to subtle noise (0 to ~200 internally max, we'll keep it low)
            const noiseVal = Math.floor((imgEl.filmGrain / 100) * 150);
            filters.push(new fabric.filters.Noise({ noise: noiseVal }));
          }
          
          // Temperature Engine (Warm/Cool)
          if (imgEl.temperature && imgEl.temperature !== 0) {
            const isWarm = imgEl.temperature > 0;
            const color = isWarm ? "#ffaa00" : "#0066ff";
            // Map temperature (-100 to 100) to an alpha value (0 to 0.4 max for subtlety)
      ...
      elements.forEach((el) => {
        ...
        if (existing) {
          ...
          if (el.type === "text" && existing.type === "textbox") {
            const textEl = el as TextElement;
            const displayContent = textEl.uppercase ? textEl.content.toUpperCase() : textEl.content;
